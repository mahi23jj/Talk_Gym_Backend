...
def upgrade() -> None:
    """Upgrade schema."""
      ...
      # 1. Create ENUM type
    attempt_stage_enum = postgresql.ENUM(
        'INITIAL', 'TRAINING', 'FINAL',
        name='attemptstage'
    )
    attempt_stage_enum.create(op.get_bind(), checkfirst=True)
    ...
    op.add_column(
        ...
        sa.Column(
            'stage',
            sa.Enum('INITIAL', 'TRAINING', 'FINAL', name='attemptstage'),
            nullable=False,
            server_default='INITIAL',
        ),
    )
    op.alter_column('attempts', 'stage', server_default=None)
...
def downgrade() -> None:
    ...
    attempt_stage_enum.drop(op.get_bind(), checkfirst=True)
    # ### end Alembic commands ###
